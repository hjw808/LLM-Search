...
def generate_queries(config: dict) -> str:
    ...
    if not config.get('enable_claude', True):
        print("Claude is disabled in configuration")
        return None

    # Check API key from environment
    api_key = os.getenv('CLAUDE_API_KEY')
    if not api_key:
        print("Error: Please set CLAUDE_API_KEY in .env file")
        return None
    ...
    try:
        ...
        print("Claude handler initialized")
    except Exception as e:
        print(f"Failed to initialize Claude handler: {e}")
        return None
...
def collect_responses(config: dict, queries_path: str, test_run_id: str = None) -> str:
    """Collect responses using Claude."""
    # Check if Claude is enabled
    if not config.get('enable_claude', True):
        print("Claude is disabled in configuration")
        return None
    ...
    print(f"Processing {len(queries)} queries with Claude...")
    ...
    def progress_callback(current, total):
        ...
        print(f"Claude progress: {current}/{total} ({percentage:.1f}%)")
    ...
    if test_run_id:
        output_filename = f"claude_responses_testrun_{test_run_id}_{timestamp}.csv"
    else:
        output_filename = f"claude_responses_{business_name}_{timestamp}.csv"
    ...
    try:
        ...
        print(f"Claude responses saved to: {output_path}")
        return output_path
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description='Claude AI Visibility Testing')
    ...
    if args.action == 'generate':
        print(f"Generating queries for: {config['business_name']} using Claude")
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        queries_filename = f"claude_queries_{business_name}_{timestamp}.txt"
        queries_path = os.path.join(business_dir, queries_filename)

        with open(queries_path, 'w', encoding='utf-8') as f:
            f.write(f"# Claude Queries for {config['business_name']}\n")
        ...
        print(f"Claude collection completed!")
