...
def generate_queries(config: dict) -> str:
    ...
    if not config.get('enable_gemini', True):
        print("Gemini is disabled in configuration")
        return None
    ...
    # Generate queries
    print("Calling Gemini API for query generation...")
...
def collect_responses(config: dict, queries_path: str, test_run_id: str = None) -> str:
    """Collect responses using Gemini."""
    # Check if Gemini is enabled
    if not config.get('enable_gemini', True):
        print("Gemini is disabled in configuration")
        return None
    ...
    print(f"Processing {len(queries)} queries with Gemini...")
    ...
    def progress_callback(current, total):
        ...
        print(f"Gemini progress: {current}/{total} ({percentage:.1f}%)")
    ...
    if test_run_id:
        output_filename = f"gemini_responses_testrun_{test_run_id}_{timestamp}.csv"
    else:
        output_filename = f"gemini_responses_{business_name}_{timestamp}.csv"
    ...
    try:
        ...
        print(f"Gemini responses saved to: {output_path}")
        return output_path
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description='Gemini AI Visibility Testing')
    ...
    if args.action == 'generate':
        print(f"Generating queries for: {config['business_name']} using Gemini")
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        queries_filename = f"gemini_queries_{business_name}_{timestamp}.txt"
        queries_path = os.path.join(business_dir, queries_filename)

        with open(queries_path, 'w', encoding='utf-8') as f:
            f.write(f"# Gemini Queries for {config['business_name']}\n")
        ...
        print(f"Gemini collection completed!")
