...
from utils.perplexity_handler import PerplexityHandler
...
def generate_queries(config: dict) -> str:
    """Generate queries using Perplexity."""
    # Check if Perplexity is enabled
    if not config.get('enable_perplexity', True):
        print("Perplexity is disabled in configuration")
        return None

    # Check API key from environment
    api_key = os.getenv('PERPLEXITY_API_KEY')
    if not api_key:
        print("Error: Please set PERPLEXITY_API_KEY in .env file")
        return None

    # Initialize Perplexity handler
    try:
        ...
        print("Perplexity handler initialized")
    except Exception as e:
        print(f"Failed to initialize Perplexity handler: {e}")
        return None
    ...
    # Generate queries
    print("Calling Perplexity API for query generation...")
...
def collect_responses(config: dict, queries_path: str) -> str:
    """Collect responses using Perplexity."""
    # Check if Perplexity is enabled
    if not config.get('enable_perplexity', True):
        print("Perplexity is disabled in configuration")
        return None
    ...
    print(f"Processing {len(queries)} queries with Perplexity...")
    ...
    def progress_callback(current, total):
        ...
        print(f"Perplexity progress: {current}/{total} ({percentage:.1f}%)")
    ...
    output_filename = f"perplexity_responses_{business_name}_{timestamp}.csv"
    ...
    try:
        ...
        print(f"Perplexity responses saved to: {output_path}")
        return output_path
    except Exception as e:
...
def main():
    parser = argparse.ArgumentParser(description='Perplexity AI Visibility Testing')
    ...
    if args.action == 'generate':
        print(f"Generating queries for: {config['business_name']} using Perplexity")
        ...
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        queries_filename = f"perplexity_queries_{business_name}_{timestamp}.txt"
        queries_path = os.path.join(business_dir, queries_filename)

        with open(queries_path, 'w', encoding='utf-8') as f:
            f.write(f"# Perplexity Queries for {config['business_name']}\n")
        ...
        output_path = collect_responses(config, args.queries)
        if not output_path:
            print("Failed to collect responses")
            sys.exit(1)

        print(f"Perplexity collection completed!")
